...
interface MPDBTableProps {
  onEdit: (record: MPDBResponse) => void
}
...
const AVAILABLE_COLUMNS = [
  ...
  { key: 'typeof_mp', title: '人力类型', width: 80 },
  { key: 'manpower', title: '人力', width: 70, align: 'right' as const },
  { key: 'machinery', title: '机械', width: 70, align: 'right' as const },
...
const MPDBTable = ({ onEdit }: MPDBTableProps) => {
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>(() => {
    ...
    return ['date', 'activity_id', 'title', 'typeof_mp', 'manpower', 'machinery', 'block', 'discipline', 'work_package']
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: reportService.deleteMPDB,
    onSuccess: () => {
    ...
    const baseColumns = visibleColumns.map(colKey => {
      ...
      return {
        ...
        render: (value: any) => {
          ...
          // 格式化 manpower 和 machinery：保持高精度，避免科学计数法
  ...
  return (
    ...
    >
      ...
      >
        <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: '#333' }}>MPDB 人力日报</h2>
        <Space size="small">
          <Button
            icon={<ReloadOutlined />}
