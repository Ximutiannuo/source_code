...
import { EditOutlined, DeleteOutlined, SettingOutlined, PlusOutlined, UploadOutlined, HistoryOutlined, InfoCircleOutlined, DownloadOutlined } from '@ant-design/icons'
...
import type { MPDBResponse } from '../types/report'
...
type MPDBInvalidActivity = {
  activity_id: string
  count: number
  manpower_total: string
  machinery_total: string
...
const AVAILABLE_COLUMNS = [
  ...
  { key: 'typeof_mp', title: 'Type of Manpower', width: 110 },
  { key: 'manpower', title: 'Manpower', width: 85, align: 'right' as const },
  { key: 'machinery', title: 'Machinery', width: 85, align: 'right' as const },
...
const MPDBPage = () => {
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>(() => {
    try {
      const saved = localStorage.getItem('mpdb-visible-columns')
      if (saved) {
  ...
  const [editingRecord, setEditingRecord] = useState<MPDBResponse | null>(null)
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['mpdb', filters, pagination.current, pagination.pageSize],
    queryFn: () =>
      reportService.getMPDB({
    ...
    enabled: location.pathname === '/mpdb', // 只在 /mpdb 页面时启用查询
  })

  const { data: invalidActivities = [], isLoading: invalidLoading } = useQuery({
    queryKey: ['mpdb-invalid-activities'],
    queryFn: () => reportService.getMPDBInvalidActivities(100),
    enabled: location.pathname === '/mpdb',
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, deletedId) => {
      ...
      queryClient.setQueryData(['mpdb', filters, pagination.current, pagination.pageSize], (old: any) => {
        if (!old) return old
        const oldItems = old.items || (Array.isArray(old) ? old : [])
        return {
          ...old,
          items: oldItems.filter((item: MPDBResponse) => item.id !== deletedId),
  ...
  const batchDeleteMutation = useMutation({
    mutationFn: async (ids: number[]) => {
      ...
      const results = await Promise.allSettled(
        ids.map(id => reportService.deleteMPDB(id))
      )
    ...
    onSuccess: (result) => {
      ...
      queryClient.setQueryData(['mpdb', filters, pagination.current, pagination.pageSize], (old: any) => {
        ...
        return {
          ...old,
          items: oldItems.filter((item: MPDBResponse) => !result.successIds.includes(item.id)),
  ...
  const handleEdit = (record: MPDBResponse) => {
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      ...
      const headerH =
        (el.querySelector('.mpdb-table .ant-table-header') as HTMLElement | null)?.getBoundingClientRect().height ?? 0
  ...
  // 当路由变化时，确保组件正确更新和清理
  useEffect(() => {
    const isMpdbPage = location.pathname === '/mpdb'
    
    if (isMpdbPage) {
      // 当进入 /mpdb 页面时，强制刷新数据
      queryClient.invalidateQueries({ queryKey: ['mpdb'] })
    } else {
      // 离开 /mpdb 页面时，立即取消所有相关查询
      queryClient.cancelQueries({ queryKey: ['mpdb'] })
    }
  ...
  // 构建默认表格列
  const defaultColumns: ColumnsType<MPDBResponse> = useMemo(() => {
    const baseColumns = visibleColumns.map(colKey => {
      ...
      return {
        ...
        align: colDef.align,
        fixed: colDef.fixed,
        ellipsis: colDef.key === 'title' || colDef.key === 'remarks' || colDef.key === 'block' ? true : undefined,
        render: (value: any) => {
          if (colDef.key === 'date') {
            return dayjs(value).format('YYYY-MM-DD')
          }
          // Activity ID 为空表示 MP 额外项，与正常作业区分展示
          if (colDef.key === 'activity_id') {
            return value != null && value !== '' ? value : '（MP额外项）'
          }
          // 格式化 manpower 和 machinery：保持高精度，避免 parseFloat 导致的精度丢失
          if (colDef.key === 'manpower' || colDef.key === 'machinery') {
            return formatHighPrecisionValue(value)
          }
          // main_block是文本，确保保留前导零（如00011显示为00011而不是11）
          if (colDef.key === 'main_block' && value != null) {
            return String(value)
          }
          return value
        },
  ...
  const invalidColumns: ColumnsType<MPDBInvalidActivity> = useMemo(() => ([
    {
      title: 'Activity ID',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      <style>{`
        /* 极致截断：锁定表格布局 */
        .mpdb-table .ant-table table {
          table-layout: fixed !important;
          width: 100% !important;
        }
        
        .mpdb-table .ant-table-cell {
        ...
        .mpdb-table .ant-table-cell > div,
        .mpdb-table .ant-table-cell .ant-table-cell-content {
        ...
        .mpdb-table .ant-table-body,
        .mpdb-table .ant-table-content { 
          overflow: auto !important; 
        }

        .mpdb-table { font-size: 12px; }
        .mpdb-table .ant-table-tbody > tr > td {
          padding: 4px 8px !important;
        }
        .mpdb-table .ant-table-thead > tr > th {
      ...
      <div className="toolbar-row">
        <h2 className="page-title">MPDB 人力日报</h2>
        <Space>
          <Input
            placeholder="作业ID"
            allowClear
          ...
          >
            <Button
              ...
              aria-label="查看 MPDB 无效 Activity ID"
      ...
      <style>{`
        /* 修复表头圆角与容器不一致的问题 */
        .mpdb-table .ant-table-thead > tr:first-child > th:first-child {
          border-top-left-radius: 4px !important;
        }
        .mpdb-table .ant-table-thead > tr:first-child > th:last-child {
      ...
      <Modal
        title="MPDB 无效作业 ID（P6不存在 / Scope不匹配）"
      ...
      <Modal
        ...
        onOk={() => {
          ...
          localStorage.setItem('mpdb-visible-columns', JSON.stringify(visibleColumns))
        }}
