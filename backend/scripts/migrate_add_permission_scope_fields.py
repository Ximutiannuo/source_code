"""
数据库迁移脚本：为permissions表添加范围字段
权限定义包含对各个模块的增删改查，也要包含对什么类型数据有访问的权限
"""
import sys
...
def migrate_add_permission_scope_fields():
    """
    为permissions表添加范围字段
    包括：scope, project, subproject, block, facility_id, work_package
    """
    db = SessionLocal()
    try:
        print("=" * 60)
        print("为permissions表添加范围字段")
        print("=" * 60)
        ...
        fields_to_add = [
            ("scope", "VARCHAR(100) NULL COMMENT 'GCC_Scope范围限制'", "description"),
            ...
            ("work_package", "VARCHAR(100) NULL COMMENT '工作包范围限制（来自rsc_defines.work_package）'", "facility_id"),
        ]
        
        for field_name, field_def, after_field in fields_to_add:
            if not check_column_exists("permissions", field_name):
        ...
        if not check_foreign_key_exists("permissions", "fk_permissions_facility_id"):
            print("  添加facility_id外键约束...")
            db.execute(text("""
                ALTER TABLE permissions 
                ADD CONSTRAINT fk_permissions_facility_id 
        ...
        indexes_to_create = [
            ("permissions", "idx_permissions_scope", "scope"),
            ("permissions", "idx_permissions_project", "project"),
            ("permissions", "idx_permissions_subproject", "subproject"),
            ("permissions", "idx_permissions_block", "block"),
            ("permissions", "idx_permissions_facility_id", "facility_id"),
            ("permissions", "idx_permissions_work_package", "work_package"),
        ]
...
if __name__ == "__main__":
    migrate_add_permission_scope_fields()
