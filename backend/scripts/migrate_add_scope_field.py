"""
数据库迁移脚本：为user_permissions和role_permissions表添加scope字段
"""
import sys
...
def migrate_add_scope_field():
    """
    为user_permissions和role_permissions表添加scope字段
    """
    db = SessionLocal()
    try:
        print("=" * 60)
        ...
        # 为user_permissions表添加scope字段
        print("\n1. 检查user_permissions表...")
        
        if not check_column_exists("user_permissions", "scope"):
            print("  添加scope字段...")
            db.execute(text("""
                ALTER TABLE user_permissions 
                ADD COLUMN scope VARCHAR(100) NULL COMMENT 'GCC_Scope范围限制' AFTER permission_id
            """))
            print("  [OK] scope字段已添加")
        else:
            print("  [-] scope字段已存在，跳过")
        
        # 为role_permissions表添加scope字段
        print("\n2. 检查role_permissions表...")
        
        if not check_column_exists("role_permissions", "scope"):
            print("  添加scope字段...")
            db.execute(text("""
